...
function Login() {
      ...
      axios
        ...
        .catch((err) => {
          if (err.response.status === 401)
            toast.error('Kiểm tra lại! Tài khoản hoặc mật khẩu không đúng. Vui lòng quay lại sau 5 phút!')
        })
        .finally(() => setLoading(false))
    } else {
      HttpClient.post('users/loginonebss/otp', {
        username,
  ...
  useEffect(() => {
    ...
    if (!isTokenExpired(token)) return navigate('/')
  }, [])
  ...
  return (
    ...
    >
      <Form layout='vertical' onFinish={handleSubmit} form={form} initialValues={{ username: '', password: '' }}>
        <Flex
          justify='center'
          style={{ padding: 16, height: isDesktopOrLaptop || isTablet ? 'calc(100vh - 96px)' : 'auto', minWidth: 300 }}
          gutter={[16, 16]}
        ...
        >
          <div>
            ...
            >
              <Flex justify='center'>
                <Flex align='center' gap={24} style={{ paddingTop: 10, paddingBottom: 16 }}>
                  <img
              ...
              <div style={{ display: !isShowOTP ? 'block' : 'none' }}>
                ...
                >
                  <Input
                    ...
                    prefix={<FaUser color={colors.gray[300]} size={16} style={{ marginRight: 6 }} />}
                  ></Input>
                </Form.Item>
                <Form.Item
                  label='Mật khẩu'
                ...
                >
                  <Input.Password
                    ...
                    style={{ padding: '10px 12px', borderRadius: 6 }}
                    prefix={<FaLock color={colors.gray[300]} size={16} style={{ marginRight: 6 }} />}
                  ></Input.Password>
                </Form.Item>
                <Form.Item>
              ...
              <Flex style={{ margin: '10px 0' }}>
                ...
                >
                  {isShowOTP ? 'Xác nhận OTP' : 'Đăng nhập bằng tài khoản OneBSS'}
                </Button>
              </Flex>
              <Divider
                align='center'
                style={{ fontSize: 14, marginTop: 20, marginBottom: 20, color: colors.gray[500] }}
              >
                {'  '} Hoặc {'  '}
              </Divider>
              <Button
                ...
                onClick={() => {
                  window.location.assign('https://id.vnpt.com.vn/cas/login?service=https://vnptnghean.com.vn')
                }}
              >
                {'Đăng nhập qua email VNPT'}
              </Button>
            </div>
            <div style={{ color: colors.blue[900], fontSize: 14, textAlign: 'center', padding: 20 }}>
              © VNPT Nghệ An
            </div>
          </div>
        </Flex>
